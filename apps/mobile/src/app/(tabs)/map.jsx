...
export default function MapScreen() {
  ...
  const [currentRegion, setCurrentRegion] = useState(null); // Mantener la región actual
  const [ignoreMapPress, setIgnoreMapPress] = useState(false); // Ignorar taps del mapa temporalmente
  const [newBoxIds, setNewBoxIds] = useState(new Set()); // Track de cajas recién creadas
  ...
  // Debounced region change handler to reduce re-renders while panning/zooming
  ...
  // Map styles (light/dark) to improve contrast and reduce visual noise
  const lightMapStyle = [
  ...
  const handleMapPress = useCallback((event) => {
    // Ignorar si acabamos de presionar un marcador
    if (ignoreMapPress) {
      return;
    }
  ...
  // Componente de marcador simplificado
  const SimpleMarker = ({ box }) => {
    const handleMarkerPress = useCallback((event) => {
      try {
        ...
        // Ignorar taps del mapa temporalmente
        setIgnoreMapPress(true);
        
        // Validar que la caja tenga los datos necesarios
        ...
        // Restaurar la capacidad de tap del mapa después de un breve delay
        setTimeout(() => {
          setIgnoreMapPress(false);
        }, 100);
  ...
  // Debug: Log boxes para verificar que se están actualizando
  console.log('Map - Current boxes:', boxes.length, boxes.map(b => ({ id: b.id, lat: b.latitude, lng: b.longitude })));
      ...
      <View style={{ flex: 1, marginTop: 100 }}>
        {location && currentRegion && (
          <MapView
            ...
            style={{ flex: 1 }}
            region={currentRegion} // Usar region en lugar de initialRegion
            onPress={handleMapPress}
            onRegionChangeComplete={debouncedSetRegion} // Mantener la región actual cuando el usuario mueva el mapa con debounce
